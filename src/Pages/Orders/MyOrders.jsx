...
const MyOrders = () => {
  ...
  const cancelOrder = async (orderId) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || "Failed to cancel order. Try again.");
    }
  };
          ...
          <AnimatePresence>
            {previousOrders
              ...
              .map((order, idx) => {
                ...
                const orderTotal =
                  order.total_amount ?? items.reduce((sum, item) => {
                      ...
                      <div>
                        <p
                          className={`font-semibold text-gray-900 ${
                            order.status === "cancelled" ? "line-through text-red-600" : ""
                          }`}
                        >
                        ...
                        <p className="text-sm text-gray-500 flex items-center gap-1">
                          <IndianRupee size={14} /> Payment: {order.payment_method}
                        </p>
                      </div>
                    ...
                    <div className="p-6">
                      <ul className="space-y-6 mb-4">
                        {items.map((item, i) => {
                          ...
                          return (
                            <li key={i} className="flex flex-col sm:flex-row justify-between gap-4 border-b pb-5">
                      ...
                      <div className="flex justify-between border-t pt-4">
                        ...
                        <p className="text-2xl font-bold text-gray-900">₹{Number(orderTotal).toLocaleString()}</p>
                      </div>
                    </div>
                  </motion.div>
                );
        ...
        >
          <div className="bg-white rounded-xl p-6 w-[90%] max-w-2xl relative shadow-lg">
            ...
            <div className="flex flex-col sm:flex-row gap-5">
              ...
              />
              <div className="flex-1">
                <h2 className="text-xl font-semibold text-gray-900">{selectedProduct.name || "Unnamed Product"}</h2>
            ...
            {selectedProduct.images && selectedProduct.images.length > 1 && (
              <div className="flex gap-2 mt-4 overflow-x-auto">
                {selectedProduct.images.slice(1).map((img, idx) => (
                  <img
                    key={idx}
                    src={img.image_url || img.images || "/placeholder-product.jpg"}
                    alt={`Extra ${idx + 1}`}
